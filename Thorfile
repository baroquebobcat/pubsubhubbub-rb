class Monk < Thor
  ...
  class Redis < Thor
    ...
    def start(env = ENV["RACK_ENV"] || "development")
      system "./vendor/redis-1.02/redis-server ./config/redis/#{env}.example.conf"
    end

    desc "test", "Start Redis server"
    def test(env = ENV["RACK_ENV"] || "test")
      system "./vendor/redis-1.02/redis-server ./config/redis/#{env}.example.conf"
    end

    desc "stop", "Stop Redis server"
    def stop(env = ENV["RACK_ENV"] || "development")
      system "killall -9 redis-server" # or ./vendor/redis-1.02/redis-cli -p 6380 shutdown
    end
  end
